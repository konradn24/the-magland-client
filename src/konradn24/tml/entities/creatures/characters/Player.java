...
//		}
	...
	public void renderGUI(long vg) {
		...
		if(lastSearchGroundTime + SEARCH_GROUND_COOLDOWN > System.currentTimeMillis()) {
			...
			nvgFillColor(vg, Colors.COLOR_COOLDOWN);
			nvgFill(vg);
		}
...
//			}
	...
	public void addExperiencePoints(int amount) {
		...
		while(level > experienceLevel) {
			...
			msg.setBarColor(Colors.COLOR_EXP_WINDOW_BAR);
			msg.setWindowColor(Colors.COLOR_EXP_WINDOW);
			
			msg.getButtonOk().setColor(Colors.COLOR_EXP_WINDOW_BUTTON);
			msg.getButtonOk().getLabel().setColor(Colors.brighten(Colors.COLOR_EXP_WINDOW_TEXT, .25f));
