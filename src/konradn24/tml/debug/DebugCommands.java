...
public final class DebugCommands {
	...
	public static String help(DebugConsole console, String... args) {
		if(args.length == 0)
	...
	public static String getCommandDetails(String commandName) {
		try {
			Method method = DebugCommands.class.getMethod(commandName, DebugConsole.class, String[].class);
	...
	public static String system(DebugConsole console, String... args) {
	...
	public static String logging(DebugConsole console, String... args) throws IOException {
	...
	public static String messageBox(DebugConsole console, String... args) {
	...
	public static String clear(DebugConsole console, String... args) {
		console.clear();
		return NULL;
	}
	
	@Command(description = "Clears command input history")
	public static String clearHistory(DebugConsole console, String... args) {
		console.clearHistory();
		return NULL;
	}
	
	@Command(description = "Returns player data")
	public static String player(DebugConsole console, String... args) {
		return handler.getPlayer().toString();
	}
	
	@Command(description = "Returns world data")
	public static String world(DebugConsole console, String... args) {
	...
	public static String add(DebugConsole console, String... args) {
		if(args.length < ADD)
		...
		boolean success = handler.getPlayer().getInventory().add(item, amount, true);
	...
	@Command(description = "Removes item from player's inventory",
			args = "<item> [amount]",
			argsHint = {"Item name or numeric ID", "Amount of specified item to remove"})
	public static String remove(DebugConsole console, String... args) {
	...
	@Command(description = "Equips provided item",
			args = "<item>",
			argsHint = "Item name or numeric ID")
	public static String equip(DebugConsole console, String... args) {
		...
		try {
			itemNumericID = Integer.parseInt(itemID);
			
			item = Item.getItem(itemNumericID);
		} catch(NumberFormatException e) {
	...
	public static String tp(DebugConsole console, String... args) {
		if(args.length < TP)
		...
		handler.getPlayer().setX(x * Tile.TILE_WIDTH);
		handler.getPlayer().setY(y * Tile.TILE_HEIGHT);
	...
	public static String game(DebugConsole console, String... args) throws IllegalArgumentException, IllegalAccessException {
		if(args.length < GAME)
	...
	public static String rendering(DebugConsole console, String... args) throws IllegalArgumentException, IllegalAccessException {
	...
	public static String spawn(DebugConsole console, String... args) throws ClassNotFoundException {
	...
	public static String kill(DebugConsole console, String... args) {
		if(args.length < KILL)
	...
	public static String killAll(DebugConsole console, String... args) {
	...
	@Command(description = "Returns nearest location of specified object",
			args = "<type> <name>",
			argsHint = {"Type of object to find (entity, biome)", "Object's name or ID (hash code)"})
	public static String find(DebugConsole console, String... args) {
		if(args.length < FIND)
		...
		if(type.equals("entity")) {
			...
			return "Found at: " + nearest.getX() / Tile.TILE_WIDTH + ", " + nearest.getY() / Tile.TILE_HEIGHT;
		} else if(type.equals("biome")) {
			int currentX = (int) (handler.getPlayer().getX() / Tile.TILE_WIDTH), currentY = (int) (handler.getPlayer().getY() / Tile.TILE_HEIGHT);
			int direction = 0; // 0 = right; 1 = down; 2 = left; 3 = up
			int moves = 1, movesLeft = 1;
			
			int i = 0;
			while(moves < handler.getWorld().getWidth() && moves < handler.getWorld().getHeight()) {	
				if(direction == 0)
					currentX += 1;
				else if(direction == 1)
					currentY += 1;
				...
				Tile tile = handler.getWorld().getTile(currentX, currentY, true);
	...
	public static String xp(DebugConsole console, String... args) {
		if(args.length < XP)
